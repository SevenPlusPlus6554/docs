...
对于这个任务来说，当然可以选择直接用普通的神经网络来做，给网络的训练数据格式为 `我 $\to$ 我/nn` 这样的多个 `单独的单词 $\to$ 标注好词性的单词`。
...
3. 隐层的神经元之间也带有权值。随着序列的推进，前面的隐层将影响后面的隐层，同时“损失”也在随序列推进而不断累积。$h_t$ 的值不仅取决于 $x_t$，还取决于 $h_{t-1}$。我们可以用下面的公式来表示循环神经网络的计算方法：
...
右侧的公式是反向传播的计算，n 个输入的误差平方和的关于每个神经元的偏导（也就是），这个过程使用的是链式求导法则。而右边是关于的偏导。

\*第一个公式：将与同一个误差时刻（红色部分，也就是普通的前馈传播造成的误差）分离出来，其他时刻的误差（绿色部分）是时序上的，这个也是造成梯度消失或爆炸的原因。

\*第二个公式，关于的分解过程，这是 BP 网络的分解方式。

\*第三个公式，关于的分解过程，这和 BP网络 的分解方式类似。
